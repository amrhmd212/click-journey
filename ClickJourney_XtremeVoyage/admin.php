...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($users as &$user) {
        if ($user['email'] === $user_email) {
            switch ($action) {
                case 'toggle_vip':
                    $user['vip_status'] = ($user['vip_status'] === 'VIP') ? 'Actif' : 'VIP';
                    break;
                case 'toggle_ban':
                    $user['vip_status'] = ($user['vip_status'] === 'Banni') ? 'Actif' : 'Banni';
                    break;
                case 'toggle_admin':
                ...
                <div class="user-actions">
                    <form method="POST">
                        ...
                        <button type="submit" class="btn vip">
                            <?= ($user['vip_status'] ?? 'Actif') === 'VIP' ? 'Désactiver VIP' : 'Activer VIP' ?>
                        </button>
                    </form>
                    <form method="POST">
                        <input type="hidden" name="user_email" value="<?= htmlspecialchars($user['email']) ?>">
                        ...
                        <button type="submit" class="btn ban">
                            <?= ($user['vip_status'] ?? 'Actif') === 'Banni' ? 'Débannir' : 'Bannir' ?>
                        </button>
                    </form>
                    <form method="POST">
                        <input type="hidden" name="user_email" value="<?= htmlspecialchars($user['email']) ?>">
