...
// Gestion de la sélection d'un pays via le formulaire
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $pays_selectionne = $_POST['pays'] ?? ''; // Récupère le pays sélectionné

    if (!empty($pays_selectionne)) { // Vérifie si un pays a été sélectionné
        // Réinitialise les données de session liées à une sélection précédente
        ...
        // Recherche le lien correspondant au pays sélectionné
        ...
        header("Location: " . $link); // Redirige vers la page des activités du pays
        exit();
    } else {
        echo "Erreur : Aucun pays sélectionné."; // Affiche une erreur si aucun pays n'est sélectionné
    }
}
...
if (isset($_GET['search'])) {
    $search = trim($_GET['search']); // Récupère la recherche depuis l'URL
}

if ($search !== "") {
    // Filtre les pays en fonction de la recherche (nom, description ou mots-clés)
    ...
    $filteredCountries = $countries; // Si aucune recherche, affiche tous les pays
}
?>
...
<body>
    ...
    <main>
        <div class="results-grid" id="resultsContainer">
            <!-- Affiche un message si aucun pays ne correspond à la recherche -->
            <?php if (empty($filteredCountries)) : ?>
                <p>Aucun pays ne correspond à votre recherche.</p>
            <?php else: ?>
                <!-- Affiche les pays correspondant à la recherche -->
