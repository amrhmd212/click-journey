...
// Démarrage de la session pour conserver les données utilisateur entre les pages
session_start();

// Récupération du nom d'utilisateur depuis la session, s'il est défini
$username = $_SESSION['username'] ?? null;

// Récupération du pays sélectionné depuis la session, s'il est défini
$paysSelectionne = $_SESSION['pays_selectionne'] ?? '';

// Vérification si une requête POST a été envoyée
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Gestion de la sélection du vol aller
    if (isset($_POST['vol'])) {
        // Stockage des informations du vol aller dans la session
        $_SESSION['vol'] = $_POST['vol'];
        $_SESSION['prix_vol'] = isset($_POST['prix_vol']) ? floatval($_POST['prix_vol']) : 0;
    }

    // Gestion de la sélection du vol retour
    if (isset($_POST['vol_retour'])) {
        // Stockage des informations du vol retour dans la session
    ...
    // Gestion de la sélection de l'hôtel
    if (isset($_POST['hotel'])) {
        // Stockage des informations de l'hôtel dans la session
        $_SESSION['hotel'] = $_POST['hotel'];
        $_SESSION['prix_hotel'] = isset($_POST['prix_hotel']) ? floatval($_POST['prix_hotel']) : 0;
    }

    // Gestion de l'ajout d'un service supplémentaire
    if (isset($_POST['service'])) {
        $service = $_POST['service'];
        $prix_service = isset($_POST['prix_service']) ? floatval($_POST['prix_service']) : 0;

        // Initialisation de la liste des options supplémentaires si elle n'existe pas
        if (!isset($_SESSION['options_supplementaires'])) {
            $_SESSION['options_supplementaires'] = [];
        }

        // Vérification si le service est déjà ajouté pour éviter les doublons
        $dejaAjoute = false;
        ...
        // Ajout du service à la liste si ce n'est pas un doublon
        if (!$dejaAjoute) {
            $_SESSION['options_supplementaires'][] = [
                'nom' => $service,
                'prix' => $prix_service
    ...
    // Gestion de la redirection vers la page de récapitulatif
    if (isset($_POST['go_to_recap'])) {
        // Vérification que toutes les informations nécessaires sont sélectionnées
        if (empty($_SESSION['vol']) || empty($_SESSION['hotel']) || empty($_SESSION['vol_retour'])) {
            ...
            // Redirection vers la page recap.php si tout est correct
...
<head>
    ...
    <!-- Inclusion des polices et des styles CSS -->
...
<body>
    <header>
        <nav>
            <ul>
                ...
                <li>
                    <!-- Affichage du bouton de connexion ou du profil selon l'état de l'utilisateur -->
                    <?php if ($username) : ?>
    ...
    <main>
        ...
        <!-- Section pour la sélection des vols aller -->
        <div class="container">
            <div class="section-title">Horaires d'Avion depuis Paris</div>
            <div class="card-container">
                <!-- Formulaire pour chaque vol aller disponible -->
                <form method="POST" class="option-form" data-type="vol">
                    <div class="schedule-item">
                        ...
                        <!-- Champs cachés pour transmettre les informations du vol -->
        ...
        <!-- Section pour la sélection des hôtels -->
        <div class="container">
            <div class="section-title">Choix d'Hôtel</div>
            <?php
            // Affichage des hôtels en fonction du pays sélectionné
            switch (strtolower($paysSelectionne)) {
                case 'portugal':
                    ...
                    <div class="country-section">
                        <h2>Portugal</h2>
                        <div class="card-container">
                            <!-- Formulaire pour chaque hôtel disponible au Portugal -->
                ...
                case 'italie':
                    ...
                    <div class="country-section">
                        <h2>Italie</h2>
                        <div class="card-container">
                            <!-- Formulaire pour chaque hôtel disponible en Italie -->
                            <form method="POST" class="option-form" data-type="hotel">
        ...
        <!-- Section pour les services supplémentaires -->
        <div class="container">
    ...
    <!-- Formulaire pour aller au récapitulatif -->
    ...
    <footer>
        <!-- Informations sur les droits et lien vers la politique de remboursement -->
